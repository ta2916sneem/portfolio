...
import {getFormattedDate, HAVE_APPLAUDED} from "../helpers";
import Level from "./Level";
import {IBlog} from "../types/blogs";

const BlogCard: FC<IBlog> = ({_id, applauds, level,
                                      publishedOn,description,
                                      title, tags}) => {

    const renderDescription = () => {
        if(!description)
            return null;
        return (<div className={"text-gray-500 text-sm flex mb-1"}>
            <div>
                {description.length > 150 ? `${description.substring(0, 151)}...` : description}
            </div>
        </div>)
    }
    ...
    const renderTags = () => {
        ...
        return (
                <div className={"flex flex-wrap text-xs pt-2"}>
                    {tags && tags.slice(0, 7).map((tag, index) => {
                        return <span key={index} className={"py-1 px-2 bg-blue-500 bg-opacity-10 mr-2 mb-2 rounded-full"}>{`${tag}`}</span>
                    })}
                    {
                        tags && tags.length > 7 && <span key={8} className={"py-1 px-2 bg-blue-500 bg-opacity-10 mr-2 mb-2 rounded-full"}>. . .</span>
                    }
                </div>
            )
    }
    ...
    return (
        <Link key={_id} to={{
            pathname: paths.readBlog,
            search: `${_id}`
        }} className={`p-4 rounded shadow-mdBlue transition duration-500 hover:shadow-xlBlue hover: ring-1 ring-blue-50 font-montserrat cursor-pointer `
            // bg-gradient-to-tr from-${getRandomColor()}-100 via-${getRandomColor()}-100 to-${getRandomColor()}-100
        }>
            <div className={"mb-2 text-2xl font-montserrat"}>
                {title}
            </div>
            <div className={"flex mb-2 justify-between items-center"}>
                <div className={"flex items-center"}>
                    {
                        !JSON.parse(localStorage.getItem(`${HAVE_APPLAUDED}${_id}`) || 'false') ?
            ...
            <div className={"text-xs text-gray-800 tracking-widest mb-2"}>
